...
parser = argparse.ArgumentParser(description="Add/replace the MEDIUM Issues tab in a governance workbook.")
...
mar_sd = datetime(2026, 3, 15, 23, 59, 59)
apr_sd = datetime(2026, 4, 15, 23, 59, 59)
may_sd = datetime(2026, 5, 15, 23, 59, 59)


def open_at(sd):
    return sum(1 for r in rows
               if (c := parse_dt(r['Created At'])) and c <= sd
               and ((rv := parse_dt(r['Resolved Time'])) is None or rv > sd))

def total_by(sd):
    return sum(1 for r in rows if (c := parse_dt(r['Created At'])) and c <= sd)

def resolved_by(sd):
    return sum(1 for r in rows
               if r['Status'].strip() == 'RESOLVED'
               and (rv := parse_dt(r['Resolved Time'])) and rv <= sd)
...
mar_res, apr_res, may_res = resolved_by(mar_sd), resolved_by(apr_sd), resolved_by(may_sd)
mar_ig,  apr_ig,  may_ig  = ignored_by(mar_sd), ignored_by(apr_sd), ignored_by(may_sd)
mar_ri,  apr_ri,  may_ri  = mar_res + mar_ig,   apr_res + apr_ig,   may_res + may_ig
...
def cdiff(a, b): d = b - a; s = '+' if d >= 0 else ''; return f'{s}{d}'
...
# Resolved issues sorted by resolved time
...
MED_FILL  = PatternFill('solid', fgColor='7030A0')   # purple for MEDIUM
...
if 'MEDIUM Issues' in wb.sheetnames:
    del wb['MEDIUM Issues']

ws = wb.create_sheet('MEDIUM Issues')
ws.sheet_properties.tabColor = '7030A0'

# Title
ws.merge_cells('A1:G1')
h(ws, 1, 1, 'MEDIUM Severity — Resolution Progress (Month-on-Month)',
  fill=MED_FILL,
...
h(ws, 2, 1, 'Exec Summary  |  Severity: MEDIUM  |  As of May 15, 2026',
...
for ri, r in enumerate(ri_issues, list_hdr_row + 1):
    ...
    for ci, val in enumerate(vals, 1):
        ...
        if ci == 3:
            fn = Font(name='Calibri', bold=True, size=10, color='7030A0')
        elif ci == 4:
...
h(ws, footer_row, 1,
  f'Total: {len(ri_issues)}  |  Resolved: {res_all}  |  Ignored: {ign_all}  |  {len(rows)-len(ri_issues)} issues still Open (not listed)',
...
print(f'Done — MEDIUM Issues tab added. {len(ri_issues)} resolved/ignored issues listed.')
